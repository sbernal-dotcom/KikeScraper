...
// mata por hard timeout externo (2026-07-29: encuentra24 y PE no dejaron
// fila cuando run-pipeline.sh los cortó a los 30/10 min).
type RunState = {
  startedAt: string;
  supa: ReturnType<typeof createScraperClient>;
  found: number;
  inserted: number;
  errors: number;
  // H4: cambio de bool → Promise. Antes: SIGTERM veía writing=true,
  // retornaba, hacía process.exit(0) → mataba el insert en curso.
  // Ahora el segundo caller await la Promise en curso y sale limpio.
...
process.on("SIGTERM", () => {
  ...
  const notes = "encuentra24 — cortado por SIGTERM del pipeline";

...
### Estabilización + defensas (2026-07-01 → 2026-07-10)

Sesión centrada en dejar el cron estable, sumar 6ª fuente y añadir defensas contra fallos silenciosos.

**Presunta venta** (`0012_presunta_venta.sql` + `marcar-presuntas-ventas.ts`, 2026-07-01):
- Columnas `presunta_venta` bool + `fecha_presunta_venta`.
- Regla: activa ≥30 días + archivada ≥14 días → probable venta.
- NO borra, solo marca. Rectifica si vuelve a activa.
- Idempotente, en el cron 1×/día.

**Validación tierra/mar** (`src/lib/geo/panama-land.ts`, 2026-07-01):
- `isOnLand(lat, lng)` — point-in-polygon contra contorno continental (~50 pts) + landmarks costeros (Coronado, Chame, Colón, Bocas…) con radio 10 km.
- Integrado en 3 puntos del pipeline (web-search, geocode-edificio, panamaequity).
- Script retroactivo `archivar-props-en-mar.ts` — 13 archivadas + 10 cache limpiadas iniciales.
- Nuevo paso 3.5 en el cron (idempotente).

**CBRE Panamá / Savitat como 6ª fuente** (`0013_savitat.sql` + `scraper-savitat.ts`, 2026-07-02):
- `cbre.com.pa` redirige a `savitat.com` — afiliado local.
- Sitemap XML ~150 URLs únicas, JSON-LD Schema.org con `geo.latitude/longitude` cuando existe.
- Comercial premium: oficinas, terrenos, industrial + inversión.
- Retry 429 en `ia-extract-edificio.ts` para el rate limit de Groq.
- Concurrency 1 + jitter 1.5-3s por respetar 6k TPM del free tier.
- **93 props insertadas** en la primera corrida.

**Precisión de ubicación + fuente auditable** (`0014_precision_ubicacion.sql`, 2026-07-02):
- Columna `precision_ubicacion` con CHECK: `'exacta' | 'zona-declarada' | 'aproximada' | null`.
- Columna `ubicacion_fuente` text libre para debug: `jsonld_geo`, `edificios_cache`, `web_search`, `titulo_zona`, `streetAddress_zona`.
- Opción `allowZoneFallback` en `geocodeConEdificio` — cuando el edificio no resuelve pero la zona sí, usar centroide SIN jitter con precision="zona-declarada". Solo Savitat lo pasa true por defecto.
- Diferente al fallback pre-strict: sin jitter (posición determinística), badge visible en UI planificado.

**Fix del cache de coords compartidas** (`limpiar-cache-duplicado.ts`, 2026-07-04):
- Descubierto: cluster de **24 edificios distintos** apuntando al mismo pin `(9.0233, -79.4986)` — gogetit.com.pa devuelve la misma coord como default. Wikipedia daba coord del artículo del barrio, no del edificio. Similar en laaparq y grupotribaldos.
- Blacklist en `buscar-edificio-web.ts`: wikipedia, gogetit, laaparq, grupotribaldos.
- Script detecta coords compartidas por >1 source_url distinto → invalida cache + archiva props (`motivo_estado='coord_falsa'`).
- Corrida inicial: **334 cache invalidadas + 471 props archivadas**.
- Nuevo paso 3.6 en el cron.

**Terrenos/casas con zona-declarada automática** (2026-07-06):
- Strict mode descartaba terrenos y casas sin edificio identificable (política "edificio o nada").
- `GeocodeOpts.categoria` — cuando es `terreno`/`lote`/`casa`, `allowZoneFallback` se activa auto sin necesidad de flag.
- Apto/oficina/local/galera se mantienen strict (son unidades DENTRO de un edificio).
- Ganancia medida: **+57 terrenos + +181 casas** que antes se perdían silenciosamente.

**Fix MLS Acobir — el sitio cambió URL de listado** (2026-07-07):
- El sitio movió `/propiedades/` → `/propiedades-en-panama/`. La URL vieja quedó como hub sin URLs individuales, solo apuntadores por categoría. 6 URLs/pág vs. 51 antes.
- El scraper corrió 3 días con 0 nuevos sin que nadie lo notara — dolor que motivó la defensa preflight.
- MAX_PAGES 30 → 100 para compensar la menor densidad.

**Optimizaciones del cron** (2026-07-08):
- Reordenado: Savitat + Verify **antes** que InmoPanama (bottleneck) — si InmoPanama explota, ya se hizo lo crítico.
- `fetchExistingUrls` de InmoPanama ahora saltea también archivadas de últimos 7 días — evita re-procesar bulk cleanups.
- Timeout workflow 120 → 150 min.
- Aplicación de las 3 fuentes = **846 activas** al final del día.

**Desastre del verify paralelo + rollback** (2026-07-09):
- Subimos VERIFY_CONCURRENCY 1 → 5 buscando bajar de 42 min a ~10 min.
- Efecto real: los portales devolvían **HTML shell sin JSON-LD** bajo bursts paralelos → clasificado como "sin Product/microdata" → contador +1 → archivado.
- **Panama Equity 58→4**, **Savitat 90→14**, **InmoPanama 121→84**. Total 846 → 695 activas (-18%).
- Rollback: concurrency 5→2, jitter 800-1600ms → 1200-2400ms.
- Script one-shot revivió **395 props** con motivo "sin Product/microdata".

**2 defensas críticas contra corrupción silenciosa** (`verificar-estado.ts` + `preflight-check.ts`, 2026-07-09 → 2026-07-10):
- **Circuit breaker en verify**: canary de 100 URLs mezcladas (recientes + antiguas + aleatorias) antes de tocar la DB. Si el ratio de "sospechosas" (200 sin Product/microdata) > 25% → **aborta todo el pase**, sin incrementar contadores ni archivar. Distingue muerte legítima (HTTP 404/410, redirect a `?prop_unavailable=`) de bug del sitio. Alerta vía GitHub Issue.
- **Preflight check por scraper**: antes de scrapear, GET al listado principal + valida ≥N URLs con patrón esperado + markers de HTML. Si falla, aborta SOLO esa fuente (las demás siguen por `continue-on-error`) + GitHub Issue con diagnóstico. Config por fuente en `PREFLIGHT_CONFIG`.
- **Fallback dominio-específico**: InmoPanama removió su JSON-LD y microdata en 2026-07 — verify ahora acepta "viva" si `<title>` contiene "InmoPanama" + HTML >20KB. Sin este fallback los 121 activos de InmoPanama se archivaban en cada corrida.
- Habría prevenido el desastre del 07-09 exacto (395 víctimas) y detectado el 07-07 el día 0 (MLS Acobir cambio de URL).

**Requeriments folder** (2026-07-02):
- Nueva carpeta `requeriments/` con 6 archivos que documentan el estado actual del sistema (complemento de bitácora que es histórica).
- README, funcional, tecnico, fuentes, ubicacion, pendientes.

**UI — imagen satelital en el grid de propiedades**:
- Aspect ratio 3/2 → 21/9 para reducir altura visual en `PropertyGridCard.tsx`.
- Se mantiene sin cambio en el detail (`PropertyCard.tsx`).

**Dedupe cross-source** — umbrales relajados (2026-07-04):
- Distancia 50m → 100m (variabilidad del web-search entre fuentes).
- Área ±5% → ±8%.
- Precio ±10% → ±15%.
- Savitat agregado al array PRIORITY.

**Fuentes evaluadas y descartadas**:
- **Compre o Alquile** — Cloudflare Turnstile agresivo bloquea Playwright headless. Requiere stealth plugin (frágil) o solver de pago. Abandonado.
- **CBRE global** — no tiene inventario en Panamá en su sitio principal (solo US/EU/Asia). Va vía Savitat (afiliado local).
- **Colliers Panamá** — sin inventario Panamá públicamente listado.
- Otros pendientes vistos rápido: RE/MAX Panamá (dominio no responde desde local).

#### Estado al final del periodo (2026-07-10)

- **~690 props activas** (drop temporal por cleanup de coords genéricas — 471 archivadas por `coord_falsa`).
- **6 fuentes activas** en el cron.
- **Cron con 2 defensas** — circuit breaker (canary) + preflight per-scraper.
...
- ✅ **Validación tierra/mar como fix de raíz** (2026-07-01): `isOnLand()` con polígono continental + landmarks costeros. Rechaza coords en el mar en 3 puntos del pipeline. Cron paso 3.5 diario.
- ✅ **Presunta venta** (2026-07-01): flag `presunta_venta` con regla temporal (≥30 días activa + ≥14 días archivada). No destructivo.
- ✅ **Savitat (6ª fuente)** (2026-07-02): CBRE Panamá afiliado, comercial premium. Sitemap XML + JSON-LD Schema.org.
- ✅ **precision_ubicacion + ubicacion_fuente** (2026-07-02): DB registra ahora nivel de precisión (exacta/zona-declarada/aproximada) y método (jsonld_geo/edificios_cache/web_search/titulo_zona…).
- ✅ **Requeriments folder** (2026-07-02): 6 archivos documentando el estado actual del sistema.
- ✅ **Coords compartidas de cache** (2026-07-04): descubierto que gogetit/wikipedia/laaparq/grupotribaldos devolvían coords genéricas — 24 edificios en el mismo pin. Blacklist + limpieza retroactiva (334 cache + 471 archivadas). Cron paso 3.6 diario para prevenir recurrencia.
- ✅ **Dedupe umbrales ajustados** (2026-07-04): distancia 50→100m, área ±5→8%, precio ±10→15%. Savitat en PRIORITY.
- ✅ **Terrenos/casas con zona-declarada auto** (2026-07-06): +57 terrenos + 181 casas que antes se descartaban silenciosamente.
- ✅ **Fix MLS Acobir cambio de URL** (2026-07-07): `/propiedades/` → `/propiedades-en-panama/`. MAX_PAGES 30 → 100.
- ✅ **Cron reordenado + timeout 150 min** (2026-07-08): Savitat + Verify antes de InmoPanama (bottleneck). skipUrls incluye archivadas de últimos 7 días para no re-procesar bulk cleanups.
- ✅ **Verify paralelo → rollback + revivir 395 víctimas** (2026-07-09): concurrency 5 causó falsos negativos masivos por HTML shell. Rollback a concurrency 2 + jitter 1200-2400ms.
- ✅ **Circuit breaker en verify** (2026-07-09): canary de 100 URLs mezcladas. Solo cuenta como sospechosas las "200 sin Product/microdata" (distingue muerte legítima). Aborta el pase + GitHub Issue si > 25%.
- ✅ **Preflight check por scraper** (2026-07-10): GET al listado + valida ≥N URLs con patrón esperado + markers. Aborta solo esa fuente si falla + GitHub Issue. Config por fuente en `PREFLIGHT_CONFIG`.
- ✅ **Fallback dominio-específico para InmoPanama** (2026-07-09): el sitio removió JSON-LD/microdata. Verify acepta viva si `<title>` contiene "InmoPanama" + HTML >20KB.
- ✅ **Cache Groq (ia_extract_cache)** (2026-07-24, migración 0015): SHA-256(titulo+desc) → resultado de extracción de edificio/zona. Evita re-consumir el rate limit de 6000 TPM re-preguntando la misma cadena. Fire-and-forget en upsert.
- ✅ **Cache URLs fallidas (urls_fallidas_cache)** (2026-07-24, migración 0016): guarda URLs que fallaron por sin_geo/404/etc con TTL 30 días. Savitat/InmoPanama/MLS las saltan en corridas siguientes sin re-consumir Groq.
- ✅ **Cron migrado a Railway** (2026-07-25): GitHub Actions cron desactivado, Railway lo maneja. Menos rate-limit issues; podemos correr más de 4h si hace falta.
- ✅ **Rediseño nb-* de InmoPanama** (2026-07-25): el sitio migró de clases `ib-prop-*` a `nb-*`, todos los selectores rompieron simultáneamente. Reescrito el extractor con `parseQuickFacts()` + fallbacks a los selectores viejos.
- ✅ **Backfill precision_ubicacion** (2026-07-25): 3781 filas históricas sin precision quedaron marcadas como "aproximada" para que la UI muestre el badge correcto.
- ✅ **Pipeline con hard cap 3h + timeouts por paso** (2026-07-27): tras deploy colgado 13h en Railway, `run-pipeline.sh` corre bajo `timeout --preserve-status --kill-after=1m 3h` con timeout individual por paso. Nunca más >3h.
- ✅ **Root fix consecutiveEmpty InmoPanama** (2026-07-28): el corte temprano usaba `nuevos.length===0` (URLs no vistas en el run) en lugar de `procesables.length===0` (nada nuevo para DB). Con 2500+ URLs en skipUrls procesaba las 50 páginas siempre. Ahora corta en 3-5 páginas típicas.
- ✅ **SIGTERM handlers en todos los scrapers** (2026-07-28→29): cuando el pipeline mata por timeout, cada scraper escribe su fila en `scraper_runs` con el progreso hasta el momento (found/inserted/errors reales). Antes se perdía la observability y no sabíamos por qué. `runState` compartido + `writeRunOnce()` en encuentra24, panamaequity, savitat, mls, inmopanama.
- ✅ **Dedupe con filtro habitaciones** (2026-07-28): antes agrupaba pisos distintos del mismo edificio con precio/área similares. Ahora rechaza pares con habitaciones distintas cuando ambos tienen dato.
- ✅ **"También publicado en" desde propiedades_duplicados** (2026-07-28): `fetchPropiedades` ahora rellena `otrosAnuncios` de la canónica leyendo `propiedades_duplicados` (la tabla `anuncios` estaba vacía). El usuario ve los precios de la misma prop en las N fuentes.
- ✅ **Savitat: fix bug zonaFallback override** (2026-07-29): la IA extraía "Área Bancaria" (no está en tabla) para un listing cuyo `streetAddress` era "Marbella, Bella Vista" (SÍ está). El `extr.zona ?? zonaFallback` tiraba la coord real. Ahora `geocode-edificio.ts` cae al `zonaFallback` cuando el centro de `extr.zona` es null.
- ✅ **Auto-alerta por email vía Resend** (2026-07-29): al final del pipeline `pipeline-alerta.ts` detecta anomalías (pipeline no corrió, fuente ausente, ≥50% errores, duración ≥150min) y manda 1 email a abilendesign@gmail.com. Silencio si todo OK. Free tier 3k/mes.
- ✅ **Página /historial** (2026-07-29): lista todas las corridas del scraper agrupadas por CORRIDA (heurístico: gap >30min = corrida nueva) — no por día. Filtros 7/30/90d y por fuente. Tabla con found/nuevas/updated/errores/eliminadas + KPIs.
- ✅ **Lifecycle visual: pin rojo apagado + banner** (2026-07-29): `fetchPropiedades` ahora trae activas + archivadas de los últimos 3 días. Pines archivados en rojo oscuro apagado (opacity 0.55). Card con banner rojo "Ya no está disponible" + motivo + fecha. Después de 3d desaparecen del mapa (quedan en DB como historial).
- ✅ **scraper_runs.archived** (2026-07-29, migración 0017): nueva columna para conteo de propiedades archivadas por corrida. `verificar-estado.ts` la escribe. UI historial muestra columna "Eliminadas" y suma en el header de cada corrida.
- ✅ **Página /scraper** (2026-07-30): expone la configuración interna del pipeline sin abrir código — KPIs en vivo, tabla de pasos con timeouts y tooltip de descripción, card por fuente con badge OK/error, sección Verify con umbrales + último resultado real, IA/APIs externas (Groq/Gemini/Mapbox/Nominatim/Resend), caches y breakdown de URLs fallidas.
- ✅ **Sidebar simplificado** (2026-07-30): quitado "Acerca de". Nav principal: Mapa · Propiedades · Análisis · Historial · Scraper.
- ✅ **Fix mapa con >700 dupIds** (2026-07-30): el `.in("id", dupIds)` generaba URL de 28KB → Bad Request de PostgREST rompía todo el mapa. Chunked queries de 200 para `otrosAnuncios` + filtrado en cliente para excluir duplicados (evita URL enorme del `.not in`).
- ✅ **Batch de 40 zonas en ZONAS_PANAMA** (2026-07-30): dry-run de MLS+Savitat mostró que 100% de los errores eran `sin_geo` por zona no reconocida. Agregadas por categoría: corregimientos PTY, avenidas frecuentes, calles numeradas, Panamá Oeste, Colón, Coclé, Veraguas, Chiriquí. También aliases y typos ("beltania"→betania).
- ✅ **Fallback por split de coma en geocode** (2026-07-30): si `"Utive, Pacora"` no matchea, prueba cada parte separada por coma. Rescata streetAddress compuestos donde el corregimiento es desconocido pero la ciudad sí.
- ✅ **MLS Acobir: allowZoneFallback + cache de fallidas** (2026-07-30): antes solo terrenos/casas activaban zone fallback → apartamentos sin edificio identificable morían silenciosos (~30% error). Ahora todas las categorías, marcadas como "zona-declarada". Además cache de URLs fallidas para no re-consumir Groq en URLs muertas cada corrida.
- ✅ **Nominatim (OSM) como último recurso con validator estricto** (2026-07-30): módulo `nominatim.ts` centralizado con `countrycodes=pa` + doble check (`address.country_code=pa` + `isOnLand()`) + validador de proximidad (rechaza hits a >30km del `zonaCentro` conocido). Sin validator devuelve null por default. Evita homónimos: "La Pulida, Panamá" caía en Darién a 350km del barrio real de Betania. Integrado como paso 5 en `geocode-edificio.ts`.
- ✅ **Fallback semántico IA en scrapers (InmoPanama + Savitat)** (2026-08-01): nuevo módulo `extraer-html-ia.ts` que — cuando los 4 regex del extractor de precio fallan — pasa un chunk del HTML crudo a Groq y le pide leer el precio directamente. Hard cap 100 llamadas/corrida, validación de rango, ancla por keyword para evitar alucinar. Diseñado para que cuando InmoPanama vuelva a rediseñar el HTML (como en 2026-07-25), el scraper NO tire error sino que use IA como último recurso. Fix del cron catastrófico 08-01 (98% error InmoPanama, 100% Savitat).
- ✅ **Refresh rotativo interno InmoPanama + Savitat** (2026-08-02): cada corrida re-visita las N más viejas por `fecha_ultima_revision` (50 en InmoPanama, 20 en Savitat) para detectar cambios de precio en anuncios ya guardados. Preserva `fecha_deteccion` original. Rotación completa en ~10 y ~5 días respectivamente. Nueva columna `scraper_runs.updated` con conteo real. Bug 2026-08-03: el corte temprano de scrapeAll de InmoPanama nunca alcanzaba las URLs viejas (están en páginas finales del listado); fixeado con `scrapeRefreshDirect()` que hace fetch directo sin depender del listado paginado.
- ✅ **Refresh liviano diario `refresh-precios.ts`** (2026-08-02): pase nuevo del pipeline que actualiza precio/área/hab/baños de TODAS las activas de TODAS las fuentes SIN tocar IA (~4-5 min/corrida). Extractores por fuente: JSON-LD, microdata o parser específico. Bug 2026-08-03: el insert a `scraper_runs.fuente_id="refresh-precios"` violaba el FK a `fuentes` → 3 días sin registrarse silenciosamente; fixeado con id="encuentra24" workaround y luego arreglado de raíz en migration 0018.
- ✅ **UI: leyenda de pines + total real del cron en sidebar** (2026-08-03): nuevo componente `MarkerLegend` con los 4 colores de pin (venta lima, alquiler naranja, cluster azul, "ya no disponible" rojo) — solo aparece en la vista de mapa. `useLastScraperRun` agora agrupa corridas por proximidad temporal (gap <30min = mismo cron) y suma inserted+updated de todas las fuentes. Nuevo color scheme: alquiler pasó de azul a naranja (#FF7A00), cluster de magenta a azul (#3B82F6).
- ✅ **Jitter visual para pines aproximados + limpieza cache mal armado** (2026-08-04): pines con `precision != "exacta"` reciben desplazamiento determinístico por-id de ±77m para que no colapsen en un pixel (cluster de 34 en San Francisco → 34 pines dispersos). Script `limpiar-cache-mal-agrupado.ts` identifica coord donde >15 propiedades caen por cache(web) mal armado (Marbella, Brisas del Golf), borra las entradas duplicadas del `edificios_cache` y marca las propiedades como prioridad refresh. Corrida real: 10 entradas de cache borradas, 48 propiedades reubicadas en 1-2 días vía refresh rotativo.
- ✅ **Fix sidebar badge "Último scrape"** (2026-08-06): dos bugs. (1) `LOOKBACK_HOURS=6` era demasiado corto — después de las 9 AM Panamá no encontraba la corrida y mostraba "Sin corridas aún"; subido a 72h. (2) Verify metía +1000 falso en "actualizadas" (bumping fechas ≠ update de datos); ahora se excluye del cálculo por match en `notes`.
- ✅ **Auditoría completa del proyecto** (2026-08-06, `AUDITORIA_2026-08-06.md`): 5 agentes de exploración paralelos cubriendo scraper correctness, security, UI/UX, data quality y tech debt. 67 findings deduplicados → 5 CRITICAL + 23 HIGH + 21 MEDIUM + 10 LOW. Seguridad limpia (RLS, secrets, XSS, robots.txt). Los 5 patrones más recurrentes: silent failures, cache poisoning permanente, duplicación masiva, cero tests, y bugs de datos por normalización floja.
- ✅ **5 CRITICAL de la auditoría cerrados** (2026-08-07): (C4 `b1db63f`) refresh-precios/verify/backfill-ia ahora tienen fila propia en `fuentes` en vez de abusar "encuentra24" (migration 0018). (C2 `7d7b28b`) helper `_lifecycle.ts` con `stripLifecycleIfNotActive` en los 6 scrapers — archivadas por verify no reviven; fetchExistingUrls trae todas las URLs (activas + archivadas). (C5 `f1d6212`) refresh-precios captura errores en el bump de fecha_ultima_revision. (C1 `2267733`) chip del mapa solo cuenta filtros que efectivamente aplica; badge para score/confianza "solo en Análisis". (C3 `99d11d4`) `normalizeKey` exportado + aplicado en los 6 `toDbRow` + backfill que consolidó 107 buckets duplicados de corregimiento (710 → 603) sobre 5757 filas históricas.
- ✅ **Fase 2 de la auditoría — 7 HIGH de silent failures cerrados** (2026-08-07): (H1 `46f32f7`) `_run-status.ts` compartido con umbral por ratio errores/total 20% — antes 200 errores + 1 inserted daba status="ok" silencioso; ahora status="error" y las alertas disparan. (H2 `4ff5620`) cada `.catch()` de `scrapeDetail` incrementa el contador de errores del run — antes 200 URLs fallando por fetch/parse/geo reportaban errors=0. (H3 `6f06f20`) `fetchExistingUrls` ahora `throw` en error de paginación en vez de retornar map parcial — evita re-procesamiento masivo Groq de URLs "creídas nuevas". (H4 `86f3d16`) `writingPromise: Promise|null` reemplaza `writing: bool` en RunState — SIGTERM handler ahora awaitea el insert en curso antes de `process.exit(0)`. (H7 `9e08311`) `fetchUrlsFallidasRecientes` + `marcarUrlFallida` en las 4 fuentes restantes (fuente-prueba, inmopanama, panamaequity, acobir) — ya no re-queman Groq en URLs sin-geo conocidas. (H16 `72f8558`) TTL 90d para hits `source='web'` en `edificios_cache` + `detectar-colapso-cache.ts` (script automático que agrupa props por coord, marca stale las entradas cache con ≥5 props colapsadas). (H17 `b7a381b`) TTL 90d en `ia_extract_cache` (soft: la fila queda pero se ignora) + `purgar-ia-cache.ts` para hard-delete manual con filtro por modelo.
- ✅ **Fase 3 auditoría — 3 HIGH scraper quality cerrados** (2026-08-09): (H5 `5ea825d`) sitemap check de Savitat con `normalizeSavitatUrl` — protocol/case/query normalizados en ambos lados; un cambio cosmético del sitemap ya no puede archivar 100+ props vivas. (H6 `7694178`) verify sostiene 3 corridas consecutivas con error antes de mover a `error_verificacion` — un 403 aislado de Cloudflare no borra estado "activo". Nueva columna `veces_error_consecutivo` (migration 0019). (H8 `37ea372`) `fetchRefreshTargets` de InmoPanama trae `tipo_operacion` real de DB — el refresh no puede cambiar venta↔alquiler silenciosamente si el badge del sitio falla.
- ✅ **Fase 4 auditoría — 4 HIGH mobile/a11y + 3 HIGH UI + 3 HIGH data quality + 3 HIGH foundation** (2026-08-10): 10 HIGH cerrados en un día. (H12 `52c0d54`) ComparisonList+PropertyCard con `w-full sm:w-[300px]` — dejan de clipear en <600px. (H13 `52c0d54`) geocoder search bar `w-[min(360px,calc(100vw-1.5rem))]` — nunca sale del viewport. (H14 `52c0d54`) pines del mapa con `role="button"` + `tabindex=0` + `aria-label` + handler Enter/Space — accesibles por teclado y lector de pantalla. (H15 `52c0d54`) color archivado `#EF4444 @ 0.7` — pasa WCAG AA. (H9 `fa8bfae`) markers del mapa no se re-crean en cada click/filter — effect de creación depende solo de `pins`, `selectedId`/`matchedIds` se aplican con `classList.toggle` sobre elementos existentes; frame time cae ~10-50×. (H10 `0325739`) overlay "Cargando propiedades…" en el primer fetch. (H11 `0325739`) `useLastScraperRun` retorna `{run, loading, error}` — badge distingue estado de red caída vs vacío legítimo. (H18 `0a8958e`) `Math.min(THRESH_ARCHIVADO, veces+1)` cap al contador. (H19 `0a8958e`) migration 0003 envuelve el `rename column` en `do $$ if exists $$` — idempotente. (H20 `0a8958e`) `reprocess-archived` actualiza `precision_ubicacion`/`ubicacion_fuente` cuando revive una prop — se acaban las filas con coord nueva y badge "aproximada" viejo. (H21 `04b1376`) **vitest + 34 tests** sobre módulos críticos (chunkedParallel, computeRunStatus, stripLifecycleIfNotActive, isOnLand). (H22 `2ed4cce`) `chunkedParallel` extraído a `scripts/scrapers/_common.ts` (8→1 copia). (H23 `2ed4cce`) `.github/workflows/ci.yml` valida tsc + tests + build + lint en cada PR/push. **Los 23 HIGH cerrados**.
- ✅ **10 LOW cerrados** (2026-08-11): (L1-L3 `4c495f9`) borrados `mock.ts` (313 líneas), `PANAMA_BOUNDS` export sin uso, `admin.ts` sin uso. (L4 `4c495f9`) `scripts/scrapers/ia.ts` reducido de 176→30 líneas — reemplazado por STUB explícito; `enriquecerConIA` retorna `ENRIQUECIMIENTO_VACIO`; removida dependency `@google/genai`. Memoria del proyecto registra que el usuario va a pasar spec de qué hace cada IA para rediseñar. (L5 `d5e62ad`) `backfill-precision-null.ts` y `backfill-corregimiento-normalizado.ts` (ya corridos) movidos a `scripts/scrapers/_archived/` con README + removidos del `package.json`. (L6 `d5e62ad`) política de comentarios en AGENTS.md: comentarios inline explican "por qué actual", historia va a bitacora.md. (L7 `4691597`) glow de alquiler `255,122,0` y cluster `59,130,246` sincronizados con sus colores base (antes eran restos del color-scheme viejo). (L8 `4691597`) `map.easeTo` con padding derecho cuando cambia `selectedId` — el pin ya no queda tapado por la card. (L9 `4691597`) `use-mobile.ts` reescrito con `useSyncExternalStore` — se acabó el flash "desktop → mobile" del primer render en celular. (L10 `4691597`) `tests/anti-pii.test.ts` con 12 tests que leen el bloque de `function toDbRow` de los 6 scrapers y fallan si aparece un key prohibido (descripcion, telefono, email, contacto, fotos, etc.) — 46/46 tests totales.
- ✅ **18/21 MEDIUM cerrados** (2026-08-11 → 08-17): (M3 `6f55c12`) verify aplica deltas de contadores después del UPDATE, no antes — stats consistentes con estado real de DB. (M4 `6f55c12`) refresh-precios lee fuentes con `SELECT DISTINCT fuente_id FROM propiedades` — nuevos scrapers se procesan automático. (M6+M7 `6f55c12`) migration 0020 con CHECK constraints en `ubicacion_fuente` (9 valores) y `scraper_runs.status` (running/ok/error). (M8 `6f55c12`) `ia_extract_cache_touch` se llama también en session-cache hits — hit_count deja de estar subestimado. (M9-M11 `7ffe1f3`) 6 claves i18n nuevas en `dict.scraper_info.verify_*` + `preview_scraped`, `title="Máx N"` usa dict, fechas pasan locale explícito. (M12 `7ffe1f3`) Skeleton component en /analisis, /historial, /propiedades, KpiCards (`…` glyph → skeleton). (M13 `7ffe1f3`) sheet de filtros abre `side="right"` para no colisionar con la Sidebar. (M15 `7ffe1f3`) `scraper.yml` agrega step `Refresh precios` que faltaba — Actions replica Railway. (M16 `7ffe1f3`) `noUncheckedIndexedAccess` probado: destapa 113 errores → dejado apagado, documentado en checklist. (M1 `b5570de`) `scripts/scrapers/extractors/inmopanama-html.ts` con `parseInmoQuickFacts` + `extractInmoPrecio` — antes 4 fallbacks de precio duplicados en scraper-inmopanama.ts y refresh-precios.ts. (M2 `b5570de`) log estructurado `[M2/isOnLand-reject]` en Savitat cuando isOnLand descarta coord JSON-LD. (M5 `b5570de`) `backfill-precision-por-fuente.ts` — re-clasifica filas de Savitat + Panama Equity que quedaron "aproximada" por el backfill masivo a "exacta" + `ubicacion_fuente="jsonld_geo"`. Idempotente. (S3 `b5570de`) `src/lib/safeHref.ts` — whitelist http/https en links externos de PropertyCard y OpportunitiesTable. (M14 `fa39c18`) **`src/lib/supabase/types.ts` regenerado desde el schema real** (10 líneas de stub → 702 líneas con 8 tablas + 2 views + enums + RPCs). Los 3 `as unknown as` reemplazados por casts simples o tipos derivados. Bug destapado: `safeCount(supabase, "edificio_coords_cache")` era un nombre inventado (la tabla real es `edificios_cache`, plural) — el card de caches en `/scraper` mostraba `—` silencioso; ahora corregido. Memoria del proyecto guarda cómo regenerar cuando cambie el schema.

## Notas Pendientes

<!-- Tareas, ideas o dudas por resolver -->

### Auditoría 2026-08-06 — 3 findings pendientes (56/59 cerrados = 95%)

Los 3 que quedan **no son código** — son configuraciones externas que solo el user puede hacer.

- ⏳ **S1 — Rate limiting en Supabase.** La `anon key` está en el bundle público (normal en apps Next), pero sin rate limits cualquiera puede martillar la API y quemar la quota. **Cómo cerrar:** Supabase Studio → Settings → API Rate Limits → setear límites por IP/token. Alternativa con más control: middleware con Upstash Redis (requiere cuenta Upstash).

- ⏳ **S2 — Token Mapbox sin restricciones de URL.** El token público está en el bundle. Sin restricciones, cualquier sitio puede usarlo → alguien podría quemar tu quota de Mapbox. **Cómo cerrar:** account.mapbox.com → Access tokens → editar el token público → agregar restricción por URL (solo los dominios donde corre la app: `mapa-interactivo-inteligente.vercel.app`, `localhost`, etc.).

- ⏳ **M16 — `noUncheckedIndexedAccess` en tsconfig.** Probado el 2026-08-11: activar el flag destapa 113 errores de tipos en 4 categorías (18048/2532/2345/2322). Arreglarlos requiere revisar caso por caso (mezclar `!` con `??` mal puede introducir bugs). Decisión: se arregla incremental al tocar cada archivo, no en batch grande. Ver `tsconfig.json`.

### Rediseño IA (pendiente del usuario)

- ⏳ **Spec de qué hace cada IA.** Al cerrar L4 (2026-08-11) se removió el módulo Gemini (`scripts/scrapers/ia.ts`) y quedó como stub. El usuario dijo que va a pasar una spec definiendo qué rol tiene cada IA (Gemini, Groq, futura opción) para rediseñar el enriquecimiento. Cuando llegue: reescribir `enriquecerConIA` sin cambiar la firma (los 8 scrapers que lo importan no se tocan). Ver [memoria `project_ia_rediseno_pendiente.md`].

### Backfills manuales pendientes

- ⏳ **`npm run backfill:precision-fuente:apply`** (fix M5). Re-clasifica filas de Savitat + Panama Equity que quedaron `precision_ubicacion="aproximada"` por el backfill masivo del 2026-07-25. Correr primero sin `:apply` (dry-run) para ver el impacto.
